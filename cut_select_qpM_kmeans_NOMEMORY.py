...
class CutSolverK(object):
    ...
    def _kmeans_clustering3(self, strat, vars_values, cut_round,filename, sel_size=0) :
        ...
        def _kmodes_clustering(df, df_pop, n_clusters=100):
            ...
            clusters = kmode.predict(df[['A','B','C']])
            df['kmodes'] = clusters

            NUM_ELEMENTS = int(100/N_CLUSTERS)
            try:
                SELECTED_CUTS = [df[df['kmodes'] == cls].sort_values(by='optimality', ascending=False).index[:NUM_ELEMENTS].values for cls in range(N_CLUSTERS)]
            except KeyError:
