...
class CutSolverK(object):
    ...
    def __init__(self):
        ...
        self._rank_list_tri = []
        self._idx_list_tri = []
        #population for PCA/ Visualization
        self._population = []
        #Keep_list of triplets used in every round
        self.keep_list=[]
        #Keep_list of some rank_list elements not used, to curry to next round
        self.keep_rank_list=[]

    def cut_select_algo(self, filename, variation, dim, sel_size, n_clusters, strat=1, nb_rounds_cuts=20, term_on=False,
        ...
        my_prob = self._my_prob  #################edw den ginetai anathesh timhs einai "function" (cplex)
        ...
        if nb_subprobs >= CutSolverK._THRES_MAX_SUBS or nb_rounds_cuts == 0:
        ...
        sel_size = min(int(np.floor(sel_size * nb_subprobs)) if sel_size < 1 else min(sel_size, nb_subprobs),
                       CutSolverK._SDP_CUTS_PER_ROUND_MAX)
        ...
        for cut_round in range(1, nb_rounds_cuts + 1):
            print('The cut round is', cut_round) #m
            ...
            if strat == 0:
                ...
                if strat != -1 and strat != 4:
                    ...
                    if variation==1:
                        rank_list=self._Temporal_1( strat, vars_values, cut_round, filename, sel_size, n_clusters)
                    if variation==2:
                        rank_list=self._Temporal_2( strat, vars_values, cut_round, filename, sel_size, n_clusters)
                    if variation==3:
                        rank_list=self._Temporal_3( strat, vars_values, cut_round, filename, sel_size, n_clusters)
                    if variation==4:
                        rank_list=self._Temporal_4( strat, vars_values, cut_round, filename, sel_size, n_clusters)
                    if variation==5:
                        rank_list=self._Temporal_5( strat, vars_values, cut_round, filename, sel_size, n_clusters)
                    if variation==6:
                        rank_list=self._Temporal_6( strat, vars_values, cut_round, filename, sel_size, n_clusters)
                    if variation==7:
                        rank_list=self._Temporal_7( strat, vars_values, cut_round, filename, sel_size, n_clusters)
                    if variation==8:
                        rank_list=self._Temporal_8( strat, vars_values, cut_round, filename, sel_size, n_clusters)

                    print("len of rank_list is", len(rank_list))
                    df=pd.DataFrame(rank_list[0:sel_size])#m 
                    ...
                    (strat_change, rank_list) = self._kmeans_clustering( strat, vars_values, cut_round, filename, sel_size , n_clusters) 
                    df=pd.DataFrame(rank_list[0:sel_size])#m
                    df_print=df_print.append(df) #m  
                else:   # for figure 8
            ...
            #print(len(vars_values)) #m
           # print('The rank list is', rank_list)#m
        ...
        #m Save to csv the rank_list from all cut rounds 
        if strat==1:
         df_print.rename(columns={0:'agg_idx',1:'set_inds',2:'-eigeval',3:'curr_pt',4:'Xarr_inds',5:'dim_act', 6:'Cluster'}, inplace=True)
        #  df_print.to_csv((r'/home/celia/Desktop/DataFrame_clustering1.csv'))#m
         df_print.to_csv((r'C:\Users\mexis\OneDrive\Υπολογιστής\SDPCutSel\Results\DataFrame_clustering.csv'))#m 
        if strat==2:
         df_print.rename(columns={0:'agg_idx',1:'set_inds',2:'obj_improve',3:'curr_pt',4:'X_slice',5:'Cluster'}, inplace=True)
         df_print.to_csv((r'/home/celia/Desktop/DataFrame_clustering2.csv'))#m  
        if strat==4:
         df_print.rename(columns={0:'agg_idx',1:'set_inds',2:'performance',3:'curr_pt',4:'X_slice',5:'Cluster'}, inplace=True)
         df_print.to_csv((r'/home/celia/Desktop/DataFrame_clustering4.csv'))#m                  
        
        if n_clusters <= 20:
            #PCA -dimensionality reduction and visualization / compare clustering methods and metrics
            population=self._population
            self._PCA(population, n_clusters)
        
        # Default behaviour (for constructing tables)
        return ([-obj for obj in curr_obj_vals], timer() - time_begin, round_times,
                sep_times, nbs_sdp_cuts, nbs_tri_cuts, vars_values, agg_list)
    ...
    def _get_sdp_vertex_cover(self, dim, ch_ext=0): 
        ...
        if len(agg_list) >= CutSolverK._THRES_MAX_SUBS:
            return len(agg_list)          
        ...
        self._agg_list = agg_list #m from here I takr the agg_list and can use it in other functions
        return len(agg_list)
    ...
    def _sel_eigcut_by_ordering_on_measure(self, strat, vars_values, cut_round, sel_size=0):
        ...
        if opt_sel or comb_sel or exact_sel:
            ...
            rank_list.sort(key=itemgetter(2), reverse=True)#m item(1) to item(2)
            # Combined selection
            if comb_sel:
                strong_violated_cuts = 0
                violated_cuts = 0
                for ix, (agg_idx,set_inds, obj_improve, curr_pt, X_slice) in enumerate(rank_list):  #m it enumerates rank_list
                    if obj_improve > CutSolverK._THRES_MIN_OPT and strong_violated_cuts < sel_size:      # strong cut
                        ...
                        if eigval < CutSolverK._THRES_NEG_EIGVAL:    # violated strong cut
                            rank_list[ix] = (agg_idx, obj_improve + CutSolverK._BIG_M, curr_pt, X_slice)
        ...
        # Feasibility (-only) selection
            ...
            for agg_idx, (set_inds, Xarr_inds, _, _) in enumerate(agg_list):
                ...
                if eigval < CutSolverK._THRES_NEG_EIGVAL:
                    rank_list[agg_idx] = (set_inds, -eigval,curr_pt, Xarr_inds, dim_act)#m add curr_pt
                    nb_violated += 1
                else:
    ...
    def _gen_eigcuts_selected(self, strat, sel_size, rank_list, strong_only=False, vars_values=None):
        ...
        while nb_sdp_cuts < sel_size and ix < sel_size:
            ...
            if not feas_sel:
                if opt_sel:
                    (idx, set_inds, diff, curr_pt, X_slice) = rank_list[ix] #m add set inds
                ...
                (idx, set_inds, _, curr_pt, Xarr_inds, dim_act) = rank_list[ix] #m add curr pt & idx &cluster
                curr_pt = itemgetter(*set_inds)(x_vals)
                X_slice = itemgetter(*Xarr_inds)(X_vals)
            ...
            #################
            if eigvals[0] < CutSolverK._THRES_NEG_EIGVAL:
                evect = np.where(abs(evect) <= -CutSolverK._THRES_NEG_EIGVAL, 0, evect)
    ...
    def __gen_dense_eigcuts(self, vars_values=None):
        ...
        ix = 0
        while ix < dim_act:
            if eigvals[ix] < CutSolverK._THRES_NEG_EIGVAL:
                evect = evecs.T[ix]
                evect_arr = [evect[idx1] * evect[idx2] * 2 if idx1 != idx2 else evect[idx1] * evect[idx2]
                             for idx1 in range(dim_act + 1) for idx2 in range(max(idx1, 1), dim_act + 1)]
                coeffs_sdp.append(cplex.SparsePair(
        ...
        for i1 in range(nb_vars):
            for i2 in range(i1 + 1, nb_vars):
                for i3 in range(i2 + 1, nb_vars):
                    ...
                    if density >= CutSolverK._THRES_TRI_DENSE:
                        set_inds = [i1, i2, i3]
    ...
    def __separate_and_add_triangle(self, sel_size, vars_values):
        ...
        rank_list_tri_viol = [el for el in rank_list_tri if el[3] >= CutSolverK._THRES_TRI_VIOL]
        ...
        nb_tri_cuts = max(min(CutSolverK._TRI_CUTS_PER_ROUND_MIN, int(np.floor(sel_size * len(rank_list_tri_viol)))),
                          min(CutSolverK._TRI_CUTS_PER_ROUND_MAX, len(rank_list_tri_viol)))
    ...
    def _Temporal_1(self, strat, vars_values, cut_round,filename, sel_size,n_clusters) :

        """Create a keep_list of rank_list elements that are not chosen 
        (10 from each cluster, +1000 in each round) cluster them also 
        (10 clusters choose 10 best)
           110 cuts per round """
        population=[]
        KL_population=[]
        n,nb_lifted, agg_list, Q, get_eigendecomp =self._nb_vars, self._nb_lifted, self._agg_list, self._Q, self._get_eigendecomp
        keep_rank_list = self.keep_rank_list
        #To vars_values prokuptei apo thn epilush tou susthmatos se kathe guro (my_prob.solve())
        X_vals, x_vals = list(vars_values[0:nb_lifted]), list(vars_values[nb_lifted:])
        rank_list = [0] * len(agg_list)
        # Guard for selection size
        sel_size = min(sel_size, len(agg_list))
        n_amb=int(sel_size/n_clusters)  #number of ambassador elements/ elements selected from each cluster for n_clusters<= sel_size
        ...
        if feas_sel:
            ...
            if n_clusters>=sel_size:
                for cluster in range(n_clusters):
                    ...
                    rank_list_new.append(rank_list_cluster[0])
                    keep_rank_list.extend(rank_list_cluster[1:11])
                self.keep_rank_list = keep_rank_list
            
                #Regenerate x_pop for keep_rank_list clustering
                for idx,(agg_idx, set_inds, pos_eigval ,curr_pt, Xarr_inds, dim_act) in enumerate(keep_rank_list):  
                    KL_x_pop=[0]*(n) 
                    curr_pt = itemgetter(*set_inds)(x_vals)                    
                    for i in range(len(set_inds)):
                        KL_x_pop[set_inds[i]]=curr_pt[i]                                                
                    KL_population.append(KL_x_pop)  

                pop_keep_list = np.asarray(KL_population)
                KL_kmeans=KMeans(n_clusters= 10, random_state=67 ).fit(pop_keep_list)
                KL_labels=KL_kmeans.labels_ 

                for cluster in range(10):
                    rank_list_cluster=[] #rank list for cluster's elements
                    for element in range(len(pop_keep_list)):
                        if KL_labels[element]==cluster:
                            rank_list_cluster.append(keep_rank_list[element])
                    amb_elem= max(rank_list_cluster, key=lambda x: x[2]) #the ambassador element of a cluster
                    rank_list_new.append(amb_elem)  #vazw sthn rank_list_new  +10 stoixeia thw keep_rank_list (10%) kai v;azw 110 periorismous se kathe guro
                #rank_list_new.sort(key=itemgetter(2), reverse=True) #system is robust to sorting the rank_list even thought all of it transforms to linear constrains

            rank_list=rank_list_new
        return rank_list
    
    def _Temporal_2(self, strat, vars_values, cut_round,filename, sel_size,n_clusters) :
        
        """New Method: with keep_list of used triplets & weights
           Remove from rank_list elements that are already on the keep_list twice and either 
           -not replace them or 
           -replace them with random element from last cluster (very few elements anyway)""" 
        print('Temporal 2')
        ...
        if feas_sel:
            ...
            #Rand_Sample = random.sample(aggidx_viol,100)


            pop_kmeans=np.asarray(population) 
            kmeans=KMeans(n_clusters= n_clusters, random_state=67 ).fit(pop_kmeans)
            labels=kmeans.labels_        
            i#nertia=kmeans.inertia_
            ...
            for i in range(len(rank_list_new)):
                if keep_list.count(rank_list_new[i][1]) >= 2:
                    print("\n Duplicate found in keep_list from previous round: ",keep_list.count(rank_list_new[i][1]),"\n",rank_list_new[i][1])
                    random.seed(67)
                    random_idx = random.randrange(len(rank_list_cluster))  #see for fix
                    rank_list_tr.append(rank_list_cluster[random_idx])
                    print("\n random index is",random_idx, "\n rand_list_tr element is:",rank_list_cluster[random_idx])
                else:
                    rank_list_tr.append(rank_list_new[i])

            keep_list.extend( [rank_list_new[i][1] for i in range(len(rank_list_new))] )
    ...
    def _Temporal_4(self, strat, vars_values, cut_round,filename, sel_size,n_clusters) : 
        ...
        """Remove used elements from population prior clustering
                (used cuts share 2/3 elements)""" 
        ###### check for problems ############
        ...
        if feas_sel:
            ...
            print("nb_violated is: ", nb_violated) 

                                
            rank_list_nonZero =[i for i in rank_list if i != 0]
            rank_list_nonZero.sort(key=itemgetter(2), reverse=True) 
            for i in range(len(rank_list_nonZero)):
                    count_doubles=0
                    r=rank_list_nonZero[i][1]
                    #print("keep_list is \n",keep_list,"\n",r)
                    for j in range(len(keep_list)):
                        k=keep_list[j]
                        if (r[0] in k and r[1] in k or r[0] in k and r[2] in k or r[1] in k and r[2] in k) :
                            count_doubles +=1
                    #print("count_doubles for element ",i ,"is:",count_doubles)
                    if count_doubles <= 2 and cut_round <=6 or count_doubles<= cut_round and cut_round>6:    ###### cut off element from 3 times in keep_list and up
                        x_pop=[0]*(n)
                        set_inds=rank_list_nonZero[i][1]
                        curr_pt = rank_list_nonZero[i][3]
                        for k in range(len(set_inds)):
                            x_pop[set_inds[k]]=curr_pt[k]                                                
                        population.append(x_pop)
             
            print("\n len of population is: ",len(population))
            ...
            for cluster in range(n_clusters):
                ...
                for element in range(len(population)):
                    if labels[element]==cluster:
                        rank_list_cluster.append(rank_list_nonZero[element])
                amb_aggidx= max(rank_list_cluster, key=lambda x: x[2])[0] #agg_idx of the ambassador element of a cluster
                rank_list_new.append(rank_list[amb_aggidx])
                keep_list.append(rank_list[amb_aggidx][1])
            rank_list_new.sort(key=itemgetter(2), reverse=True) #system is robust to sorting the rank_list even thought all of it transforms to linear constrains
    ...
    def _Temporal_5(self, strat, vars_values, cut_round,filename, sel_size,n_clusters) :
        
        """
        keep a list of used cuts
        move a number of non used & unique cuts from one iteration to another with a rank_keep_list
        (non unique cuts share 2/3 elements)
        """ 
        population=[]
        n,nb_lifted, agg_list, Q, get_eigendecomp =self._nb_vars, self._nb_lifted, self._agg_list, self._Q, self._get_eigendecomp
        keep_list= self.keep_list
        ...
        if feas_sel:
            ...
            rank_list_nonZero =[i for i in rank_list if i != 0]
            rank_list_nonZero.extend(keep_rank_list)
            
            rank_list_nonZero.sort(key=itemgetter(2), reverse=True) #need to sort first (feasibility rank) and then cut off multiple elements

            #formulate population: x_pop for rank_list + keep_rank_list
            for i in range(len(rank_list_nonZero)):
                x_pop=[0]*(n)
                set_inds=rank_list_nonZero[i][1]
                curr_pt = rank_list_nonZero[i][3]
                for j in range(len(set_inds)):
                    x_pop[set_inds[j]]=curr_pt[j]                                                
                population.append(x_pop)
            ...
            for cluster in range(n_clusters):
                ...
                for element in range(len(population)):
                    if labels[element]==cluster:
                        rank_list_cluster.append(rank_list_nonZero[element])
                top_rank_elem = max(rank_list_cluster, key=lambda x: x[2])
                rank_list_new.append(top_rank_elem)
                keep_list.append(top_rank_elem[1])  #keep_list consists of the set_inds of the chosen cuts 
            rank_list_new.sort(key=itemgetter(2), reverse=True) #system is robust to sorting the rank_list even thought all of it transforms to linear constrains

            #remove chosen cuts from rank_list_nonZero
            for i in range(len(rank_list_new)):
                rank_list_nonZero.remove(rank_list_new[i])

            
            counter=0
            #keep_100 unique cuts for next rounds
            for i in range(len(rank_list_nonZero)):
                    count_doubles=0
                    r=rank_list_nonZero[i][1]
                    #print("keep_list is \n",keep_list,"\n",r)
                    for j in range(len(keep_list)):
                        k=keep_list[j]
                        if (r[0] in k and r[1] in k or r[0] in k and r[2] in k or r[1] in k and r[2] in k) :    ## 2/3 common elements are examined
                            count_doubles +=1
                    #print("count_doubles for element ",i ,"is:",count_doubles)
                    if count_doubles <= 2:    ###### cut off element from 3 times in keep_list and up
                        counter+=1                                             
                        keep_rank_list.append(rank_list_nonZero[i])
                    if counter==100:
                        break
            ...
            self.keep_rank_list=keep_rank_list
            print("\n len of keep_list is: ",len(keep_list))
            print("\n len of keep_rank_list is: ",len(keep_rank_list))

        return rank_list_new

    def _Temporal_6(self, strat, vars_values, cut_round,filename, sel_size,n_clusters) :
        
        """Dynamic Method: Scan rank_list_cluster for element that satisfies the constrain 
        (n+2 for 2/3 and n for 3/3) 
        and if not found, gradually relax constrain""" 
                ...
                for n in range(20):
                    if counter == 0:
                        for i in range(len(rank_list_cluster)):
                            ...
                            for j in range(len(keep_list)):
                                k=keep_list[j]
                                if (r[0] in k and r[1] in k or r[0] in k and r[2] in k or r[1] in k and r[2] in k) :  # 2/3 common dimensions
                                #if (r[0] in k and r[1] in k and r[2] in k):  # all 3 common dimensions
                                    #print("duplicate triplet found\n",r,"\n",k)
                                    count_doubles +=1
                            #print("count_doubles for element ",i ,"of cluster", cluster,"is:",count_doubles)
                            if count_doubles <= n+2:
                            #if count_doubles <= n:  
                                tup = (count_doubles,i)
                                count_doubles_list.append(tup)
                                rank_list_new.append(rank_list_cluster[i])
                                keep_list.append(rank_list_cluster[i][1])
                                counter += 1
                                break
            
            print("count_doubles_list is:", count_doubles_list)
    ...
    def _Temporal_7(self, strat, vars_values, cut_round,filename, sel_size,n_clusters) :
        
        """Dynamic Method: Scan 10 first elements of rank_list_cluster for element that satisfies the constrain 
        (n+2 for 2/3 and n for 3/3) 
        and if not found, gradually relax constrain""" 
        ...
        if feas_sel:
            ...
            for cluster in range(n_clusters):
                ...
                for n in range(20):
                    if counter == 0:
                        range_num= len(rank_list_cluster) if len(rank_list_cluster) < 10 else 10
    ...
    def _Temporal_8(self, strat, vars_values, cut_round,filename, sel_size,n_clusters) :
        
        """Spectrum Method: with keep_list of used triplets ( 2 common elements ), but for every dimension take x_{n-1}~= x_{n}~= x_{n+1}
           """ 
        ...
        if feas_sel:
            ...
            for cluster in range(n_clusters):
                ...
                for i in range(len(rank_list_cluster)):
                    ...
                    r_new=[]
                    for i in range(3):
                        r_new.append([r[i]-1,r[i],r[i]+1])
                    #print("keep_list is \n",keep_list,"\n",r)
                    for j in range(len(keep_list)):
                        k=keep_list[j]
                        common_elem=0
                        for z1 in range(len(r_new)):
                            for z2 in range(len(r_new[z1])):
                                if r_new[z1][z2] in k:
                                    common_elem+=1
                        if common_elem >=2 :
                            #print("duplicate triplet found\n",r,"\n",k)
                    ...
                    #if count_doubles <= 2 and cut_round <=6 or count_doubles<= (cut_round+1) and cut_round>6:    ###### cut off element from 3 times in keep_list and up
                    if count_doubles <= 2 and cut_round <=6 or count_doubles<= 1000 and cut_round>6: #(cut_round) and cut_round>5:   #prepei na vrw thn optimum tolerance
                        rank_list_new.append(rank_list_cluster[i])
                        keep_list.append(rank_list_cluster[i][1])
                        break
            rank_list_new.sort(key=itemgetter(2), reverse=True) #system is sensitive to sorting the rank_list even thought all of it transforms to linear constrains
